...
void KleinKramers2d::Evolve()
{
    ...
    if ( !isFullGrid )
    {
        ...
        for (int i1 = EDGE; i1 < BoxShape[0]-EDGE; i1 ++)  {
            for (int i2 = EDGE; i2 < BoxShape[1]-EDGE; i2 ++)  {
                b1 = F[i1*W1+i2] < TolH;
                b2 = std::abs(F[(i1+1)*W1+i2] - F[(i1-1)*W1+i2]) < TolHdX1;
                b3 = std::abs(F[i1*W1+(i2+1)] - F[i1*W1+(i2-1)]) < TolHdX2;
    ...
    for (int tt = 0; tt < (int)(TIME / kk); tt ++)
    {
        ...
        if ( !isFullGrid )
        {
            ...
            for (int i = 0; i < TB.size(); i++)
            {
                ...
                g2 = (int)(TB[i] % M1);
                b1 = F[g1*W1+g2] >= TolL;
                b2 = std::abs(F[(g1+1)*W1+g2]*TAMask[(g1+1)*W1+g2] - F[(g1-1)*W1+g2]*TAMask[(g1-1)*W1+g2]) >= TolLdX1;
                b3 = std::abs(F[g1*W1+(g2+1)]*TAMask[g1*W1+(g2+1)] - F[g1*W1+(g2-1)]*TAMask[g1*W1+(g2-1)]) >= TolLdX2;
                b4 = g1 > EDGE && g2 > EDGE;
